...
- модель загружена некорректно и не содержит `feature_names_in_`: возвращается понятная `500` ошибка
- ошибка инференса модели не роняет процесс сервиса и возвращается как `500 Prediction failed`
...
- при смене модели метаданные активной модели, списка её фич и drift-окна корректно сбрасываются и пересоздаются
...
В качестве основного хранилища метрик используется `Prometheus`. Дополнительно сервис отправляет дублирующий поток метрик в `Graphite`, но dashboards и alerting для сдачи строятся вокруг `Prometheus default`.
...
Создан Telegram-бот для отправки уведомлений в отдельный групповой чат с алертами. При сдаче работы к ссылке на Pull Request прикладывается пригласительная ссылка на этот чат, чтобы проверяющий мог увидеть фактическую доставку уведомлений.
...
Набор правил, используемый для демонстрации на защите:

- `ML Service Down`
- `ML Service High Error Rate`
- `ML Predict High Latency`
- `ML Required Features Missing`
- `ML Data Drift Detected`
- `ML Service High Memory Usage`
- `ML Prediction Class Skew`
...
Для локальной разработки dashboards поднимаются автоматически через provisioning в контейнере Grafana. Для выданной учебной Grafana dashboards импортируются вручную из JSON-файлов репозитория с привязкой к `Prometheus default`.

Импортированы следующие dashboards:
...
При импорте dashboard-файлов datasource сопоставляется с `Prometheus default`, поэтому dashboards повторно переносимы между локальной Grafana и учебной Grafana.
...
## 7. Как проверялось решение

Ручная проверка выполнялась следующими сценариями:

- запуск сервиса и проверка `GET /health`
- запуск тестов внутри контейнера `ml_service`
- генерация нагрузки на `POST /predict` для наполнения drift-окон
- проверка `GET /metrics`, `GET /drift/status`, `GET /drift/report`, `GET /drift/report/html`
- проверка RemoteWorkspace-синхронизации при наличии `EVIDENTLY_URL` и `EVIDENTLY_PROJECT_ID`
- импорт dashboards во внешнюю Grafana
- настройка Telegram contact point
- ручной запуск сценариев, вызывающих firing alert rules
